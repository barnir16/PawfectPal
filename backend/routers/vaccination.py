...
def update_vaccination(
    ...
    db_vaccination = db.query(VaccinationORM).join(PetORM).filter(
        VaccinationORM.id == vaccination_id,
        PetORM.user_id == current_user.id
    ).first()
...
def get_pet_vaccination_summary(
    ...
    vaccinations = db.query(VaccinationORM).filter(VaccinationORM.pet_id == pet_id).all()
    
    total_vaccinations = len(vaccinations)
    today = date.today()
    
    # Find next due date
    future_due_dates = [v.next_due_date for v in vaccinations if v.next_due_date and v.next_due_date >= today]
    next_due_date = min(future_due_dates) if future_due_dates else None
    
    # Count overdue vaccinations
    overdue_count = len([v for v in vaccinations if v.next_due_date and v.next_due_date < today])
    
    # Check if up to date (no overdue vaccinations)
    up_to_date = overdue_count == 0
    
    # Get completed series (vaccines with no pending doses)
    completed_series = list(set([
        v.vaccine_name for v in vaccinations 
...
def get_vaccinations_due_soon(
    ...
    for v in vaccinations:
        ...
        reminders.append(VaccinationReminder(
            vaccination_id=v.id,
            pet_id=v.pet_id,
            pet_name=v.pet.name,
