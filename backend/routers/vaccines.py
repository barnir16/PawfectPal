...
async def get_pet_vaccination_summary(
    pet_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
...
async def update_vaccination(
    ...
    try:
        ...
        vaccination = (
            db.query(Vaccination)
            .join(Pet)
            .filter(Vaccination.id == vaccination_id, Pet.user_id == current_user.id)
            .first()
        )
...
async def get_overdue_vaccinations(
    ...
    try:
        ...
        vaccinations = (
            db.query(Vaccination)
            .join(Pet)
            .filter(Pet.user_id == current_user.id, Vaccination.next_due_date < today)
            .all()
        )
