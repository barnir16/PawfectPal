...
def get_pet_medical_records(
    ...
    records = query.order_by(MedicalRecordORM.date.desc()).offset(offset).limit(page_size).all()
...
def update_medical_record(
    ...
    db_record = db.query(MedicalRecordORM).join(PetORM).filter(
        MedicalRecordORM.id == record_id,
        PetORM.user_id == current_user.id
    ).first()
...
def get_pet_medical_summary(
    ...
    # Find next follow-up
    future_followups = [r.follow_up_date for r in records if r.follow_up_date and r.follow_up_date > date.today()]
