...
def create_service_request(
    ...
    if not service_type_obj:
        available_services = ", ".join(st.name for st in db.query(ServiceTypeORM).all())
        raise HTTPException(
            status_code=400,
            detail=(
                f"Service type '{request.service_type}' does not exist. "
    ...
    if not available_providers:
        raise HTTPException(
            status_code=400,
            detail=(
                f"No providers are currently offering '{request.service_type}' service. "
                "Please try a different service type."
            ),
        )
...
def get_service_request(
    ...
    is_public_open = request.status == "open"

    if not (
        is_owner
        or is_assigned_provider
        or (is_public_open and current_user.is_provider)
    ):
...
def assign_provider(
    ...
    logger.info("Assigned provider to service request %s", request_id)
