...
def create_provider_profile(
    ...
    if existing_profile:
        raise HTTPException(
            status_code=400, detail="User already has a provider profile"
        )

    # Validate service types exist
    service_types = (
        db.query(ServiceTypeORM)
        .filter(ServiceTypeORM.id.in_(profile.service_type_ids))
        .all()
    )

    if len(service_types) != len(profile.service_type_ids):
        raise HTTPException(status_code=400, detail="Some service type IDs are invalid")
...
def get_provider_profile(profile_id: int, db: Session = Depends(get_db)):
    """Get a specific provider profile by ID"""

    profile = (
        db.query(ProviderProfileORM).filter(ProviderProfileORM.id == profile_id).first()
    )
...
def update_my_provider_profile(
    ...
    if service_type_ids is not None:
        ...
        service_types = (
            db.query(ServiceTypeORM)
            .filter(ServiceTypeORM.id.in_(service_type_ids))
            .all()
        )

        if len(service_types) != len(service_type_ids):
            raise HTTPException(
                status_code=400, detail="Some service type IDs are invalid"
            )
...
def get_service_types(
    ...
    return [
        ServiceTypeSchema(id=st.id, name=st.name, description=st.description)
